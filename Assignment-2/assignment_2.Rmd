...
# Saving Information For A3
```{r}
head(my_DE_results)

my_DE_results$Gene <- rownames(my_DE_results)

write.csv(
  my_DE_results,
  file = "my_DE_results.csv",
  row.names = FALSE,    # Because we moved rownames into "Gene"
  quote = FALSE
)
```
```{r}
# Ensure the gene identifiers are stored in a column called 'Gene'
my_DE_results$Gene <- rownames(my_DE_results)

# Create a ranked gene list using the formula: rank = -log10(PValue) * sign(logFC)
my_DE_results <- my_DE_results %>%
  mutate(rank = -log10(PValue) * sign(logFC)) %>%
  arrange(desc(rank))

# Save the full DE results for later use in Assignment 3
write.csv(
  my_DE_results,
  file = "CD_DE_results.csv",
  row.names = FALSE,
  quote = FALSE
)

# Create a .rnk file for GSEA (with no header, as required by GSEA's preranked mode)
# The file will contain two columns: Gene and rank.
write.table(
  my_DE_results[, c("Gene", "rank")],
  file = "CD_deg.rnk",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
  col.names = FALSE  # GSEA expects no header line
)
